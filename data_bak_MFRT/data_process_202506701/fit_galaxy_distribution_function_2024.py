...
# os.makedirs("savefig/"+output_folder_name, exist_ok=True)
output_folder_name = cpgi.read_output_folder_name_from_user_settings(is_only_first=True) #model1 name
...
fitmassdensity_tag, fitAA_tag, is_preprocess_rotation = cpgi.read_fitmassdensity_tag_from_IC_setting_list(name_MG)
#\() Enable is_preprocess_rotation to be Fault for triaxialize file in galaxy_general/aa/ with [0.,0.,0.]
add.DEBUG_PRINT_V(1, fitmassdensity_tag, fitAA_tag, is_preprocess_rotation, "fitmassdensity_tag, fitAA_tag, is_preprocess_rotation")
...
def main_step2_DF(tag, snapshot_ID, is_read=False, is_grid=False):
    ...
    if int(tag)==1:
        ...
        RG.AAAA_set_particle_variables(
            ...
            col_particle_IDs=7, col_particle_type=7, col_particle_mass=8, col_particle_potential=14
        )
        ...
        particle_type = RG.particle_type #mask_select_type
        N_dim = RG.system_space_dimension
        N_ptc = RG.system_particles_count
        add.DEBUG_PRINT_V(1, np.shape(x), np.shape(v))
        # spin_L_old_direct = add.calculate_spin_L_before_preprocess(np.hstack((x, v)), mass, pot_old)
        ...
        xv = np.hstack((x, v))
        bd = bd1
        xv_cl, cl, cln = add.screen_boundary_some_cols(xv, cols, -bd, bd, value_discard=None)
    ...
    if int(tag)==2:
        ...
        bd = bd2
        AA_cl, cl, cln = add.screen_boundary_some_cols(AA, cols, 1e-2, bd, value_discard=None) #the AA has been changed after this function
        # AA_cl, cl, cln = add.screen_boundary_some_cols(AA, cols, 1e-2, bd, value_discard=bd*1e20) #the AA has been changed after this function
        # AA_cl, cl, cln = add.screen_boundary_some_cols(AA, cols, 1./bd, bd, value_discard=bd*1e4)
        ps = AA #now: ps, mass, cl, meds
        # spin_L_processed_direct = add.calculate_spin_L_before_preprocess(xv=data[:,0:6], mass=data[:,7], pot=data[:,-4])
        ...
        # add.DEBUG_PRINT_V(1, 1e-2, bd)
        # add.DEBUG_PRINT_V(1, np.max(Act))
        # i_Act = np.argmax(np.sum(Act, axis=1))
        # add.DEBUG_PRINT_V(1, np.max(Fre))
        # add.DEBUG_PRINT_V(1, np.max(AA))
        # i_bad = np.argmax(np.sum(AA, axis=1))
        # add.DEBUG_PRINT_V(1, data[i_bad], data[i_bad, 78:78+10])
        # add.DEBUG_PRINT_V(1, i_Act, i_bad, Act[i_bad], Fre[i_bad], AA[i_bad])
        # add.DEBUG_PRINT_V(0, np.max(AA_cl), len(AA_cl))
    ...
    if is_read:
        ...
        if not is_grid:
            ...
            if ye is not None:
                ye = ye[cl]
            # add.DEBUG_PRINT_V(0, np.max(AA_cl), np.max(tgts), "max_AA")
        else:
            ## [select]: samples: grid
            N_grid = 100000
        ...
        if is_actionerror and int(tag)==2:
            ...
            if os.path.exists(fne):
                ...
                # add.DEBUG_PRINT_V(0, np.shape(xe), np.shape(ye), np.mean(ye), np.mean(np.log10(DF)), ye)
            else:
...
def main_step4_fit_DF_x_mass(ps, mass, tgts, DF, xe, ye, cl, meds, MG, param00):
    ...
    axisratio_direct = add.axis_ratio_by_configuration_data(ps[:,0:3]) #tgts
    
    vinp_err, vout_err = xe, ye
    binsxy = pgw.plot_1d_DF_by_bins(vinp[:, 0:3], is_show=is_show)

    paramsresult1 = {
        "scale_length_comp1_fit": -1, "scale_length_comp1_median_direct": -1, 
        "scale_density_comp1_fit": -1, "flatx_comp1_direct": -1, 
        "flaty_comp1_direct": -1, "flatz_comp1_direct": -1, 
        "fitmodel_name_xv_fit": "-1", "powerS1_fit": -1, 
        "powerA1_fit": -1, "powerB1_fit": -1, 
        "powerA2_fit": -1, "powerB2_fit": -1, 
        "spin_L_old_direct": -1, "beta_sigma_total_direct": -1
    }
    #then from ps to direct
    paramsresult1["scale_length_comp1_median_direct"] = np.median( add.norm_l(ps[:,0:3], axis=1) )
    paramsresult1["flatx_comp1_direct"] = axisratio_direct[0]
    paramsresult1["flaty_comp1_direct"] = axisratio_direct[1]
    paramsresult1["flatz_comp1_direct"] = axisratio_direct[2]
    paramsresult1["spin_L_old_direct"] = param00[0] #no use
    paramsresult1["beta_sigma_total_direct"] = add.calculate_beta_sigma_total_from_xv(ps) #no use
    ...
    #after fitting, send value from the fit MG to total param
    paramsresult1["scale_length_comp1_fit"] = MG.params_dict["length_scale"][0]
    paramsresult1["scale_density_comp1_fit"] = MG.params_dict["density_scale"][0]
    paramsresult1["fitmodel_name_xv_fit"] = fitmodel[0][1]
    paramsresult1["powerS1_fit"] = -1. #no use in fact
    paramsresult1["powerA1_fit"] = MG.params_dict["power_free_1"][0]
    paramsresult1["powerB1_fit"] = MG.params_dict["power_free_2"][0]
    paramsresult1["powerA2_fit"] = -1.
    paramsresult1["powerB2_fit"] = -1.

    #: should unpack DL
    return DL, combmodel, fitmodel, paramsresult1

def main_step5_fit_DF_AA_one(ps, mass, tgts, DF, xe, ye, cl, meds, MG, param00):
    ####[] tables for params, with axisRatio, ds, rs, 
    # time power, common shape, nothing and gJ and colliSize
    ...
    Js_data = meds[3][2]
    Js_expected = np.sqrt(gm.G*M*ls)
    axisratio_action_direct = add.axis_ratio_by_configuration_data(vinp) #tgts
    ...
    paramsresult2.update({"spin_L_processed_direct": param00[1]}) #no use
    ...
    fitmodel = [ # label_code_about_adjust_fit_function
        # [
        #     gm.DFAA_fCombinationFreq_DPL_Gauss_log, 
        #     "DFAA_fCombinationFreq_DPL_Gauss_log",
        #     [
        #         "power_free_1", "power_free_2", 
        #         "scale_free_1", "scale_free_2", "scale_free_3", 
        #         "log_penalty"
    ...
    paramsresult2.update({"fitmodel_name_AA_fit": fitmodel[0][1]})
    ...
    #: should unpack DL
    return DL, combmodel, fitmodel, paramsresult2
...
def assign_fitvalues_auto_to_MG(values_order, names, MG, col=0):
    n = len(names)
    for i in np.arange(n):
        MG.params_dict[names[i]][col] = values_order[i]
    return 0

def main_step4_mpfit_fit_DF_x_mass(ps, mass, tgts, DF, xe, ye, cl, meds, MG, param00, is_fit_1d=True, fitmassdensity_tag=-1):
    ####[] tables for params, with axisRatio, ds, rs, 
    ...
    paramsresult1.update({"scale_length_comp1_median_direct": np.median( add.norm_l(ps[:,0:3], axis=1) )})
    ...
    paramsresult1.update({"beta_sigma_total_direct": add.calculate_beta_sigma_total_from_xv(ps)})
    ...
    # np.savetxt("tmp_data/1.txt", add.merge_array_by_hstack([h,vout]))
    # add.DEBUG_PRINT_V(0, np.mean(vinp, axis=0))
    # bd = bd1
    # h, vout = add.screen_remove_far_back(h, vout, False) #optional if back bad points and write
    hbd, fbd, febd, xsbs, nxss = add.split_various_bin_1d_percentile(h, vout, None, [0., 0.01, 0.99, 1.], [10, 100, 10], True) #curve
    # hbd, fbd, febd, xsbs, nxss = add.split_various_bin_1d_percentile(h, vout, None, [0., 1.], [1000], False) #curve
    # hbd, fbd, febd, xsbs, nxss = add.split_various_bin_1d_percentile(h, vout, None, [0., 1.], [1000], True) #curve
    # plt.figure()
    # plt.scatter(h, vout)
    # plt.scatter(hbd, fbd)
    # plt.xscale("log")
    # plt.show()
    # plt.close()
    add.DEBUG_PRINT_V(1, np.max(h), np.max(h))
    ...
    #: fit arguments for each fitmodel
    rs0 = paramsresult1["scale_length_comp1_median_direct"]
    rhos0 = M/rs0**3
    add.DEBUG_PRINT_V(1, rs0, rhos0, "rs0, rhos0")
    power1_0 = 1.
    power2_0 = 3.
    ...
    # p0_1 = np.array([ rs0, rhos0, power1_0 ])
    ...
    #: select fitmodel and fit (here the combmodel is absorbed)
    for j in [fitmassdensity_tag]:
        p0 = p0_pack[j]
        bounds = bounds_pack[j]
        add.DEBUG_PRINT_V(1, fitmassdensity_tag, fitmodel[j][0], p0, bounds[0], bounds[1])
        MF = fgw.MP_fit(fitmodel[j][0], p0, 
            xdata=hbd, ydata=fbd, errdata=None, bounds=bounds, 
            quiet=0)

        mparams = MF.run()
        fitmodel[j][4] = mparams
        assign_fitvalues_auto_to_MG(fitmodel[j][4], fitmodel[j][2][:-1], MG)
        # add.DEBUG_PRINT_V(0, mparams)
    ...
    paramsresult1.update({"fitmodel_name_xv_fit": fitmodel[0][1]})
    ...
    return DL, combmodel, fitmodel, paramsresult1, hbd, fbd, h

def main_step5_mpfit_fit_DF_AA_one(ps, mass, tgts, DF, xe, ye, cl, meds, MG, param00, is_fit_1d=True, value_vectors_extern=None):
    ####[] tables for params, with axisRatio, ds, rs, 
    
    #: processed data
    vinp, vout = tgts, np.log10(DF) #xdata and ydata after log10
    AA_sum = np.sum(vinp[:, 0:3], axis=1)
    Js_data = meds[3][2]
    Js_expected = np.sqrt(gm.G*M*ls)
    axisratio_action_direct = add.axis_ratio_by_configuration_data(vinp) #tgts
    ...
    h = gm.AA_combination_sumWeightFrequency_rateF1(vinp) #might be zero
    # h = np.abs(gm.AA_combination_sumWeightFrequency_rateF1(vinp)) #might be zero
    # np.savetxt("tmp_data/2.txt", add.merge_array_by_hstack([h,vout]))
    # add.DEBUG_PRINT_V(0, np.mean(vinp, axis=0)) #[8e3, 4e3, 2e3, 14., 13., 11.]
    # bd = bd2
    h, vout = add.screen_remove_far_back(h, vout, h_far=5e5, is_mask_log10f=False) #optional if back bad points and write
    # h, vout = add.screen_remove_far_back(h, vout, h_far=1e6, is_mask_log10f=False) #optional if back bad points and write
    hbd, fbd, febd, xsbs, nxss = add.split_various_bin_1d_percentile(h, vout, vout_err, [0., 0.01, 0.99, 1.], [20, 100, 20], True) #curve
    # add.DEBUG_PRINT_V(0, np.shape(fbd), np.shape(febd))
    # hbd, fbd, febd, xsbs, nxss = add.split_various_bin_1d_percentile(h, vout, vout_err, [0., 0.01, 0.99, 1.], [20, 60, 20], True) #curve
    # hbd, fbd, febd, xsbs, nxss = add.split_various_bin_1d_percentile(h, vout, vout_err, [0., 0.01, 0.99, 1.], [100, 100, 3], True) #curve #sometimes for Einasto
    # hbd, fbd, febd, xsbs, nxss = add.split_various_bin_1d_percentile(h, vout, vout_err, [0., 0.01, 0.95, 1.], [10, 100, 10], False) #curve
    # plt.figure()
    # plt.scatter(h, vout)
    # plt.scatter(hbd, fbd)
    # plt.xscale("log")
    ...
    if is_fit_1d:
        fitmodel = [
            ...
            #     "fh_SPL_log10", 
            #     [
            #         "scale_free_2", "scale_free_3", "scale_free_4", 
            #         "power_free_2", "power_free_3", 
            #         "coef_free_1", "coef_free_2", "coef_free_3", "coef_free_4", "coef_free_5", 
            #         "log_penalty"
            #     ], 
            #     None, #the fixed params
            #     None, #the value of unfixed fit params
    ...
    for j in np.arange(N_fitmodel):
        ...
        # p0 = get_arguments_auto_from_MG(fitmodel[j][2], MG.params_dict)[:-1]
        # bounds = [get_arguments_auto_from_MG(fitmodel[j][2], MG.params_dict, 2)[:-1], 
        #     get_arguments_auto_from_MG(fitmodel[j][2], MG.params_dict, 3)[:-1]]
        # add.DEBUG_PRINT_V(1, fitmodel[j][0], p0, bounds[0], bounds[1])
        # MF = fgw.MP_fit(fitmodel[j][0], p0, 
        #     xdata=h, ydata=vout, errdata=vout_err, bounds=bounds, 
        #     quiet=0)

        #EinastoUsual 2e5, fh_P1PL_Exp_log10
        p0 = np.array([
        ...
        # #EinastoUsual 2e5, fh_P1PL_log10
        # p0 = np.array([
        ...
        if is_actionerror:
            MF = fgw.MP_fit(fitmodel[j][0], p0, 
                ...
                maxiter=1000, quiet=0) #?? bad fit result
        else:
            MF = fgw.MP_fit(fitmodel[j][0], p0, 
                xdata=hbd, ydata=fbd, errdata=None, bounds=bounds, 
    ...
    return DL, combmodel, fitmodel, paramsresult2, hbd, fbd
...
def plot_simply_density_fitting(
    ...
    plt.savefig(pathfig)
    print("Plot and save a figure. Done.")
    return 0

def plot_simply_actions_fitting(
        JOb, DF_log10, fit_function, fit_params_list, pathfig, 
        auxiliary_function_plot2d_x=None, JO1=None, DF1_log10=None, is_bin=True
):
    xd = auxiliary_function_plot2d_x(JO1)
    h1 = auxiliary_function_plot2d_x(JO1)
    xdb = JOb
    yd = DF1_log10
    ydb = DF_log10
    yf = fit_function(h1, *fit_params_list) #nan value
    ...
    plt.legend()
    plt.xscale("log")
    plt.xlabel(r"value of actions dot frequencies, $h_J$ ($\mathrm{kpc\, km/s/Gyr)}$")
    plt.ylabel(r"log10 value of NDFA, f(J) ($\mathrm{1/(kpc\, km/s)^3)}$")
    plt.savefig(pathfig)
    print("Plot and save a figure. Done.")
    return 0

def write_paramsresult(result_file_info_path, paramsresult, tag):
...
def read_and_compare_params_debug(prefix_name, mgs_name, suffix_name):
    ...
    for j in np.arange(N_paramfiles_name): #before preprocessed
        ...
        # add.DEBUG_PRINT_V(0, paramfiles_name, "paramfiles_name[j]")
    ...
    # add.DEBUG_PRINT_V(1, value_vectors_1, value_vectors_2)
...
def read_and_compare_params_vectors(paramfiles_name, params_name, suffix_name, x_name=None, plot_index_of_params=None):
    ...
    np.savetxt(folder_params_statistics+suffix_name+".txt", value_vectors, header=header)
    print("Save file to %s. Done."%(folder_params_statistics))
    ...
    fig = None
    as_x = np.argsort(x)
    # add.DEBUG_PRINT_V(0, as_x, x[as_x], x[as_x][0], x[as_x][-1])
    plt.figure(dpi=300)
...
if __name__ == '__main__':
    ...
    if tag_task==1: #xv triaxialization
        ...
        # three times of no use triaxialization, should optimized ??
        x, v = ps[:,0:3], ps[:,3:6] #note: without cl
        fitmassdensity_tag = fitmassdensity_tag #?? 0: DPL; 1: Einasto; 2: MDPL
        # DL, combmodel, fitmodel, paramsresult = main_step4_fit_DF_x_mass(ps, mass, tgts, DF, xe, ye, cl, meds, MG, param00)
        DL, combmodel, fitmodel, paramsresult, hbd, fbd, h = main_step4_mpfit_fit_DF_x_mass(ps, mass, tgts, DF, xe, ye, cl, meds, MG, param00, fitmassdensity_tag=fitmassdensity_tag)
        path_file_data = galaxyfit_name+"snapshot_%d.fit.txt"%(snapshot_ID)
        pathfig = path_file_data+".halodensity.png"
        p_rho = fitmodel[fitmassdensity_tag][4]
        # if fitmodel[fitmassdensity_tag][3] is not None:
        #     p_rho = np.append(fitmodel[fitmassdensity_tag][4], fitmodel[fitmassdensity_tag][3])
        plot_simply_density_fitting(hbd, fbd, fitmodel[fitmassdensity_tag][0], p_rho, pathfig, x, np.log10(DF), h)
        ...
        # DL, combmodel, fitmodel, paramsresult, hbd, fbd = \
        #     main_step5_mpfit_fit_DF_AA_one(
        #         ps, mass, tgts, DF, xe, ye, cl, meds, MG, 
        #         param00, is_fit_1d=True, value_vectors_extern=value_vectors_extern
        #     )
        # path_file_data = galaxyfit_name+"snapshot_%d.fit.txt"%(snapshot_ID)
        # pathfig = path_file_data+".png"
        # plot_simply_actions_fitting(hbd, fbd, fitmodel[0][0], fitmodel[0][4], pathfig, gm.AA_combination_sumWeightFrequency_rateF1, tgts, np.log10(DF))
        # write_paramsresult(path_file_data, paramsresult, tag=2)

        for gadget_type in mask_select_type:
            DL, combmodel, fitmodel, paramsresult, hbd, fbd = \
                main_step5_mpfit_fit_DF_AA_one(
                    ps, mass, tgts, DF, xe, ye, cl, meds, MG, 
                    param00, is_fit_1d=True, value_vectors_extern=value_vectors_extern
                ) #?? add arguments
            path_file_data_types = galaxyfit_name+"snapshot_%d.fit_%d.txt"%(snapshot_ID, gadget_type)
        ...
        # add.DEBUG_PRINT_V(0, paramfiles_name)
        prefix_name = folder_name+"galaxy_general_"
        for i in np.arange(len(mgs_name)):
            paramfiles_name[i] = prefix_name+mgs_name[i]+"/fit/snapshot_%d.fit.txt"%(snapshot_ID)
        suffix_name = mgs_name[0]+"_etal"
        add.DEBUG_PRINT_V("suffix_name, ", suffix_name)

        # val1, val2 = read_and_compare_params_debug(prefix_name, mgs_name, suffix_name)
        # add.DEBUG_PRINT_V(0, val1, val2, "someparam")
        ...
        #\ do not use this function when compare singular galaxy model

        params_name = gm.params_name
        x_name = None
        # x_name = "powerAA_P1_fit"
        # x_name = "powerAA_P2_fit"
        plot_index_of_params = None #one should changed this each time
        # plot_index_of_params = [0,1,2] #one should changed this each time
    ...
    #### debug
        ...
        path_file_data = galaxyfit_name+"snapshot_%d.fit.txt"%(snapshot_ID)
        pathfig = path_file_data+".png"
        # add.DEBUG_PRINT_V(0, pathfig)
        # plot_simply_actions_fitting(tgts, np.log10(DF), fitmodel[0][0], fitmodel[0][4], pathfig, gm.AA_combination_sumWeightFrequency_rateF1, tgts, np.log10(DF))
        plot_simply_actions_fitting(hbd, fbd, fitmodel[0][0], fitmodel[0][4], pathfig, gm.AA_combination_sumWeightFrequency_rateF1, tgts, np.log10(DF))
        write_paramsresult(path_file_data, paramsresult, tag=2)
