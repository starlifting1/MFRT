...
def main_step2_DF(tag, snapshot_ID, is_read=False, is_grid=False):
    ...
    if int(tag)==1:
        ...
        RG.AAAA_set_particle_variables(
            ...
            col_particle_IDs=7, col_particle_mass=8, col_particle_potential=14
        )
    ...
    if int(tag)==2:
        ...
        mass = RG.particle_mass
        IDs = RG.particle_IDs
        particle_type = RG.particle_type ##particle_type is written by C++
...
def main_step4_fit_DF_x_mass(ps, mass, tgts, DF, xe, ye, cl, meds, MG, param00):
    ...
    paramsresult1 = {
        ...
        "fitmodelfunc_name_xv_fit": "-1", "powerS1_fit": -1, 
    ...
    paramsresult1["scale_length_comp1_median_direct"] = np.median( ads.norm_l(ps[:,0:3], axis=1) )
    ...
    paramsresult1["beta_sigma_total_direct"] = ads.calculate_beta_sigma_total_from_xv(ps) #no use
    ...
    for j in np.arange(N_fitmodelfunc):
        WF = fgw.Wrap_func(MG, fitmodelfunc[j][0], fitmodelfunc[j][2], fitmodelfunc[j][3]) #-log_panalty
        ...
        fitmodelfunc[j][4] = CF_optimization
        # #[adjust] fit
        # p_hand = [5.e-2,     1., 1.e-1, 1., 2.,  2.e5]
        # CF_ymodel = fitmodelfunc[j][0](vinp, *p_hand)
    ...
    paramsresult1["fitmodelfunc_name_xv_fit"] = fitmodelfunc[0][1]
    ...
    #: should unpack DL
    return DL, combmodel, fitmodelfunc, paramsresult1
...
def main_step5_fit_DF_AA_one(ps, mass, tgts, DF, xe, ye, cl, meds, MG, param00):
    ...
    fitmodelfunc = [ # label_code_about_adjust_fit_function
        # [
    ...
    #: fit (here the combmodel is absorbed)
    for j in np.arange(N_fitmodelfunc):
        names_for_fit = fitmodelfunc[j][2] if fitmodelfunc[j][2] is not None else list(gm.params_name)
        WF = fgw.Wrap_func(MG, fitmodelfunc[j][0], names_for_fit) #-log_penalty
        CF = fgw.Curve_fit_galaxy(WF, vinp, vout, vout_err, mf=5000)
        CF.run() #WF.funcfit, WF.reference_list
        CF_optimization, CF_covariance, CF_residual_sigma, CF_error_fit, CF_ymodel = CF.display()
        fitmodelfunc[j][4] = CF_optimization
        ...
        for i in np.arange(N_combmodel):
            ...
            VOther = [CF_residual_sigma, CF_error_fit, 
                Js_data, Js_expected, 
                meds] #[7]
            # ads.DEBUG_PRINT_V(1, Js_data, Js_expected)
    ...
    #: record basic fitted parameters
    ...
    #: record other fitted parameters for user-defined models (and defaults) #??
    # _names_AA = [nm for nm in fitmodelfunc[0][2] if nm != "log_penalty"]
    # paramsresult2["fitted_params_names_AA"] = _names_AA
    # for nm in _names_AA:
    #     if nm in MG.params_dict:
    #         paramsresult2[f"{nm}_fit"] = MG.params_dict[nm][0]

    _names_AA = [nm for nm in (fitmodelfunc[0][2] or []) if nm and nm != "log_penalty"]
    paramsresult2["fitted_params_names_AA"] = _names_AA
    # Prefer the freshly fitted vector from the model tuple [* , * , names, *, values, *]
    if fitmodelfunc and fitmodelfunc[0] and len(fitmodelfunc[0]) > 4 and fitmodelfunc[0][4]:
        _vals_AA = [float(x) for x in fitmodelfunc[0][4]][:len(_names_AA)]
    else:
        # Fallback: pull current values from MG if available
        _vals_AA = [float(MG.params_dict.get(k, [0.0])[0]) for k in _names_AA]
    paramsresult2["fitted_params_values_AA"] = _vals_AA
    # Back-compat: expose per-name *_fit (col 0); also make sure MG has them
    for k, v in zip(_names_AA, _vals_AA):
        if k not in MG.params_dict:
            MG.params_dict[k] = [0.0]
        if not MG.params_dict[k]:
            MG.params_dict[k].append(0.0)
        MG.params_dict[k][0] = v
        paramsresult2[f"{k}_fit"] = v

    #: should unpack DL
    return DL, combmodel, fitmodelfunc, paramsresult2
...
def main_step4_mpfit_fit_DF_x_mass(ps, mass, tgts, DF, xe, ye, cl, meds, MG, param00, is_fit_1d=True, fitmassdensity_tag=-1):
    ...
    return DL, combmodel, fitmodelfunc, paramsresult1, hbd, fbd, h
...
def main_step5_mpfit_fit_DF_AA_one(
    ...
    for j in np.arange(N_fitmodelfunc):
        ...
        if is_fit_1d:
            ...
            # ## action state density 1d: forward-model bins, evaluate the DF on *each particle's* h(J),
...
if __name__ == '__main__':
    ...
    if tag_task==1: #xv triaxialization
        ...
        DL, combmodel, fitmodelfunc, paramsresult, hbd, fbd, h = main_step4_mpfit_fit_DF_x_mass(ps, mass, tgts, DF, xe, ye, cl, meds, MG, param00, fitmassdensity_tag=fitmassdensity_tag)
        ...
        plot_simply_density_fitting(hbd, fbd, fitmodelfunc[fitmassdensity_tag][0], p_rho, pathfig, x, np.log10(DF), h)
        ...
        ## particle_type array appended in param00[2] by main_step2_DF(tag=2)
        ptype = None
        if (param00 is not None) and (len(param00) >= 3):
            ptype = param00[2]
        # ads.DEBUG_PRINT_V(0, len(ptype), "len(ptype)")
    ...
    #### debug
        ...
        gadget_type = 1 #default halo
        path_file_data = galaxyfit_name+"snapshot_%d.type_%d.fit.txt"%(snapshot_ID, int(gadget_type))
        pathfig = path_file_data+".png"
        # ads.DEBUG_PRINT_V(0, pathfig)
        # plot_simple_actions_fitting(fitmodelfunc[0][0], fitmodelfunc[0][4], pathfig, tgts, np.log10(DF), gm.AA_combination_sumWeightFrequency_rateF1, tgts, np.log10(DF))
        plot_simple_actions_fitting(fitmodelfunc[0][0], fitmodelfunc[0][4], pathfig, hbd, fbd, gm.AA_combination_sumWeightFrequency_rateF1, tgts, np.log10(DF))
    ...
    #### XV
        ...
        # ##[] step4 fit DF_x_mass
        # DL, combmodel, fitmodelfunc = main_step4_fit_DF_x_mass(ps, mass, tgts, DF, xe, ye, cl, meds, MG)
        # main_step6_record_and_plot_fitvalues(MG, DL, combmodel, fitmodelfunc, tag=1)
    ...
    #### AA
        ...
        ##[] step6 fit DF_AA_one
        DL, combmodel, fitmodelfunc = main_step5_fit_DF_AA_one(ps, mass, tgts, DF, xe, ye, cl, meds, MG)
        
        # special_init_guess = DL[0][0][6] #cf opt
        # info = [output_folder_name+"/"+"snapshot_%d"%(snapshot_ID)]
        # DL, combmodel, fitmodelfunc = main_step5_fit_mc_DF_AA_one(ps, mass, tgts, DF, xe, ye, cl, meds, MG, info=info, special_init_guess=special_init_guess)
        
        main_step6_record_and_plot_fitvalues(MG, DL, combmodel, fitmodelfunc, tag=2)
