...
class Wrap_func:
    ...
    def funcfit(self, x, *p): # list of params may not be all of the params of self.funcfit_formula
        # alpha, beta, Js = p[0:self.N_fitparams-1]
        # return self.funcfit_formula(x, alpha, beta, Js)
...
class Wrap_func_rho_dsrs(Wrap_func):
    def set_bounds(self, params_name=[""]):
        self.p_other = None
        self.reference_list = [self.model.params_dict['power1'][1], self.model.params_dict['power2']
                               [1], self.model.params_dict['density_scale'][1], self.model.params_dict['length_scale'][1]]
        self.min_list = [self.model.params_dict['power1'][2], self.model.params_dict['power2'][2],
                         self.model.params_dict['density_scale'][2], self.model.params_dict['length_scale'][2]]
        self.max_list = [self.model.params_dict['power1'][3], self.model.params_dict['power2'][3],
                         self.model.params_dict['density_scale'][3], self.model.params_dict['length_scale'][3]]

    def funcfit(self, x, *p):  # to fix some other params
        alpha, beta, ds, rs = p[0:self.N_fitparams-1]
        return self.funcfit_formula(x, alpha, beta, ds, rs)

    def log_prior(self, theta):
        alpha, beta, ds, rs = theta[0:-1]
        log_f = theta[-1]
        # print(theta)
        if self.min_list[0] < alpha < self.max_list[0] \
                and self.min_list[1] < beta < self.max_list[1] \
                and alpha < beta \
                and self.min_list[2] < ds < self.max_list[2] \
                and self.min_list[3] < rs < self.max_list[3] \
                and self.model.params_dict['log_penalty'][2] < log_f < self.model.params_dict['log_penalty'][3]:
            return 0.
        else:
            # print("Out of prior!")
...
class Wrap_func_rho_Einasto_nmlz(Wrap_func):
    def set_bounds(self, params_name=[""]):
        self.p_other = None
        self.reference_list = [self.model.params_dict['power_Einasto'][1],
                               self.model.params_dict['density_scale'][1], self.model.params_dict['length_scale'][1]]
        self.min_list = [self.model.params_dict['power_Einasto'][2],
                         self.model.params_dict['density_scale'][2], self.model.params_dict['length_scale'][2]]
        self.max_list = [self.model.params_dict['power_Einasto'][3],
                         self.model.params_dict['density_scale'][3], self.model.params_dict['length_scale'][3]]

    def funcfit(self, x, *p):  # to fix some other params
        # plE, ds, rs = p[0:self.N_fitparams-1]
        # return self.funcfit_formula(x, plE,ds,rs)
        # self.funcfit = galaxy_models.rho_spherical_Einasto_polygamma_log
        # params = self.p_other
        params = list(range(self.N_fitparams))  # plE, ds, rs, log_f
        for i in range(self.N_fitparams-1):
            params[i] = p[i]
        return self.funcfit_formula(x, params[0], params[1], params[2])
    ...
    def log_prior(self, theta):
        ...
        for i in range(self.N_fitparams):
            params[i] = theta[i]
        # print(theta)
        if self.min_list[0] < params[0] < self.max_list[0] \
                and self.min_list[1] < params[1] < self.max_list[1] \
                and self.min_list[2] < params[2] < self.max_list[2] \
                and self.model.params_dict['log_penalty'][2] < params[-1] < self.model.params_dict['log_penalty'][3]:
            return 0.
        else:
            # print("Out of prior!")  # np.log(0)
            return -np.inf

    def log_likelihood(self, theta, x, y, yerr):  # can be put into base class
        abc = theta[0:-1]
        log_f = theta[-1]
        ymodel = self.funcfit(x, *abc)
        ye = yerr
    ...
    def log_posterior(self, theta, x, y, yerr):  # can be put into base class
        lp = self.log_prior(theta)
        if not np.isfinite(lp):
            return -np.inf
        ll = self.log_likelihood(theta, x, y, yerr)
...
class Wrap_func_rho_doublepowerlaw_triaxial(Wrap_func):
    def set_bounds(self, params_name=[""]):
        if not len(params_name)==nf:
            print("Wrong length of params! Please check.")
            exit(0)
        self.params_name = params_name
        self.p_other = None
    ...
    def log_prior(self, theta):
        for i in range(self.N_fitparams): # prior of params, one by one
            if not( self.model.params_dict[self.params_name[i]][2] < theta[i] < self.model.params_dict[self.params_name[i]][3] ):
                # print("Out of prior!")
                return -np.inf
        # if not theta[0]<theta[1]:
        #     return -np.inf
...
class Wrap_func_fJ_sph_sph_2pl(Wrap_func):
    def set_bounds(self, params_name=[""]):
        self.p_other = None
        for i in range(self.N_fitparams-1):
            self.reference_list[i]  = self.model.params_dict[params_name[i]][1]
    ...
    def log_prior(self, theta):
        alpha, beta, Js = theta[0:-1]
        log_f = theta[-1]
        # print(theta)
        if self.model.params_dict['power1'][2] < alpha < self.model.params_dict['power1'][3] \
                and self.model.params_dict['power2'][2] < beta < self.model.params_dict['power2'][3] \
                and alpha < beta\
                and 0. < Js < self.model.params_dict['action_scale'][2]*1.e3 \
                and self.model.params_dict['log_penalty'][2] < log_f < self.model.params_dict['log_penalty'][3]:
            return 0.
        else:
            # print("Out of prior!")
...
class MCMC_fit_galaxy:  # 故意找茬是不是
    def __init__(self, wf, nf, x, y, yerr, nw=60, ns=2000, nb=1000, nth=1, nd=0, sd=0):
        self.Wrap = wf  # params constraints and fit model
        self.N_fitparams = nf  # counts of params
        self.x = x  # argumanet x
        self.y = y  # argumanet y
        self.yerr = yerr  # argumanet y error
        self.N_walkers = nw
        self.N_steps = ns
        self.N_burn = nb
        self.N_thin = nth
    ...
    def display(self, name="", dpi=300, is_show=0):
        # #() corner, plot, save
        # #() readjust params distribution
        # emcee_trace = self.result.reshape(-1, self.N_fitparams).T #-1: the rest of shape
        # self.params_MC = np.zeros(self.N_fitparams)
        # for kk in range(self.N_fitparams):
        #     self.params_MC[kk] = np.median(emcee_trace[kk]) #np.mean
        # ymodel = self.Wrap.funcfit(x,*self.params_MC[0:-1])
        # self.error_fit = [add.norm_l(ymodel,y, axis=0) / len(y)]
        # print("MCMC fit result: ", self.params_MC, self.error_fit)
        # return self.params_MC, self.error_fit
        ...
        fig_tmp = plt.gcf()
        fig_tmp.savefig("savefig/MCMC/steps_"+name, format='png', dpi=dpi) #'eps' 'pdf' 'png'
        if is_show == 1:
            plt.show()
        ...
        fig_tmp = plt.gcf()
        fig_tmp.savefig("savefig/MCMC/corner_"+name, format='png', dpi=dpi)
        if is_show == 1:
            plt.show()
        print("Fig ... Done.")
        plt.close("all")
...
class Curve_fit_galaxy:
    def __init__(self, wf, nf, x, y, yerr, tag0=0, mf=2000):
        self.Wrap = wf
        self.N_fitparams = nf
        self.maxfev = mf
        self.x = x
    ...
    def display(self):
        ymodel = self.Wrap.funcfit(self.x, *self.optimization)
        self.error_fit = [add.norm_l(ymodel, self.y, axis=0) / len(self.y)]
        self.residual = ymodel-self.y
        self.residual_sigma = np.std(self.residual)
        ...
        return self.optimization, self.covariance, self.residual, self.residual_sigma, self.error_fit


class Read_data_galaxy:
    # galaxybox_name = "/home/darkgaia/workroom/0prog/gadget/Gadget-2.0.7/"

    def __init__(self, m, gb="/home/darkgaia/workroom/0prog/gadget/Gadget-2.0.7/", gmn="galaxy_general/", wc=0):
        self.model = m
...
def plot_one_snapshot(snapshot_Id, galaxymodel_name, MG, WF, is_fit=1, projection_surface="xy", is_show=0):
    ...
    RD = Read_data_galaxy(MG, 2, gmn=galaxymodel_name)
...
if __name__ == '__main__':
    ...
    for k in range(len(PM)):
        PM[k] = []
    DD = [[[0 for k in range(len(DD_name))] for i in range(N_combmodel)] for j in range(N_fitmodel)] #decrease dimension and plot
