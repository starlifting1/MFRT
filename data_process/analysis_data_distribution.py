...
def screen_boundary_some_cols(AA, cols_judge, bd_low, bd_up, value_discard=None):
    '''
    Warning: It is not deep copy and it may change the input when enable value_discard.
    '''
    n1 = len(AA)
    n2 = len(cols_judge)
    ...
    if value_discard is not None:
        AA[condition_list_not] = value_discard #this will change the inputed AA
...
# -----------------------------
# Helpers: weighted quantiles
# -----------------------------
def _weighted_quantile(values, quantiles, sample_weight=None):
    """Weighted quantile(s) of 1D values. quantiles in [0,1]."""
...
# ---------------------------------------------------------
# 1) Piecewise x-bin edges by percentiles (robust, standard)
# ---------------------------------------------------------
def make_x_edges_percentile_piecewise(
    x,
    percentile_split=(0.0, 0.01, 0.99, 1.0),
    n_bins_arr=(10, 100, 10),
    is_geomspace=True,
    x_must_be_positive_for_geom=True,
):
    """
    Build x-edges by splitting x into percentile ranges, then making bins in each range.
    Returns:
      x_edges (1D, length = total_bins + 1)  [monotonic, unique]
      cut_each_percent (right edge of each percentile block)
      n_bins_each_block (actual bins produced per block)
    Notes:
      - For geomspace, each block needs x_lo>0 and x_hi>0.
      - Uses finite x only for percentile bounds; edges apply to all.
    """
    x = np.asarray(x, dtype=float)
    pct = np.asarray(percentile_split, dtype=float)
    nb = list(n_bins_arr)
    # print(pct, nb)
    if len(pct) < 2 or len(nb) != (len(pct) - 1):
        raise ValueError("percentile_split length must be len(n_bins_arr)+1")

    xf = x[np.isfinite(x)]
    if xf.size == 0:
        raise ValueError("No finite x values.")
    xf_sorted = np.sort(xf)

    def _pct_value(p):
        # p in [0,1]
        idx = int(np.floor(p * (xf_sorted.size - 1)))
        idx = max(0, min(idx, xf_sorted.size - 1))
        return xf_sorted[idx]

    edges_blocks = []
    cut_each = np.zeros(len(nb), dtype=float)
    n_bins_each = np.zeros(len(nb), dtype=int)

    for i in range(len(nb)):
        x_lo = _pct_value(pct[i])
        x_hi = _pct_value(pct[i + 1])
        cut_each[i] = x_hi

        if not np.isfinite(x_lo) or not np.isfinite(x_hi) or (x_hi <= x_lo):
            # degenerate: skip this block
            continue
        ...
        if is_geomspace:
            if x_must_be_positive_for_geom and (x_lo <= 0.0 or x_hi <= 0.0):
                # fall back to linear inside this block
                e = np.linspace(x_lo, x_hi, n_bins + 1)
            else:
                e = np.geomspace(x_lo, x_hi, n_bins + 1)
        else:
            e = np.linspace(x_lo, x_hi, n_bins + 1)

        # keep only finite and strictly increasing edges
        e = e[np.isfinite(e)]
        e = np.unique(e)
        if e.size >= 2:
            edges_blocks.append(e)
            n_bins_each[i] = e.size - 1

    if not edges_blocks:
        raise ValueError("No valid blocks produced edges.")

    # stitch blocks, removing duplicate boundary between blocks
    x_edges = edges_blocks[0]
    for e in edges_blocks[1:]:
        if np.isclose(e[0], x_edges[-1]):
            x_edges = np.concatenate([x_edges, e[1:]])
        else:
            x_edges = np.concatenate([x_edges, e])

    x_edges = np.unique(x_edges)
    if x_edges.size < 2:
        raise ValueError("Degenerate x_edges after stitching.")

    return x_edges, cut_each, n_bins_each


# ---------------------------------------------------------
# 2) Heatmap in (log10 h, log10 f) with optional weights
# ---------------------------------------------------------
def hist2d_xy(
    x_log10,
    y_log10,
    ...
    """
    2D histogram on (x_log10, y_log10).
    Returns a dict with:
      H      : counts (or sum of weights)      shape (Nx, Ny)
      H_w2   : sum of weights^2 (if weights)  shape (Nx, Ny) else None
      N      : raw counts                      shape (Nx, Ny)
      x_edges, y_edges
    """
    x = np.asarray(x_log10, dtype=float)
    y = np.asarray(y_log10, dtype=float)
    ...
    if weights is None:
        N, _, _ = np.histogram2d(x, y, bins=[xe, ye])
        return {"H": N, "H_w2": None, "N": N, "x_edges": xe, "y_edges": ye}
    ...
    H_w2, _, _ = np.histogram2d(x, y, bins=[xe, ye], weights=w * w)
    N, _, _ = np.histogram2d(x, y, bins=[xe, ye])
    return {"H": H, "H_w2": H_w2, "N": N, "x_edges": xe, "y_edges": ye}


def make_y_edges_linear(y_log10, n_y_bins=160, y_range=None, clip_percentiles=(0.2, 99.8)):
    """
    y_edges for log10(f) axis.
    If y_range is None, uses percentiles for robust bounds.
    """
    y = np.asarray(y_log10, dtype=float)
    yf = y[np.isfinite(y)]
    if y_range is None:
        if yf.size == 0:
            # fallback: keep code running even if all y are NaN/Inf (common in mpfit trial steps)
            lo, hi = -1.0, 1.0
        else:
            lo, hi = np.percentile(yf, [clip_percentiles[0], clip_percentiles[1]])
    else:
        lo, hi = y_range
    if (not np.isfinite(lo)) or (not np.isfinite(hi)) or (hi <= lo):
        lo, hi = -1.0, 1.0
    return np.linspace(lo, hi, int(n_y_bins) + 1)

# -------------------------------------------------------------------
# 3) Extract a 1D ridge curve (for mpfit) from the heatmap or points
# -------------------------------------------------------------------
def ridge_from_heatmap(
    heatmap_dict,
    ridge="mode",
    min_count_per_xbin=30,
    q_lo=0.16,
    q_hi=0.84,
    return_full=False,
):
    """
    For each x-bin column, collapse the y-distribution to one representative y.

    ridge:
      - "mode": y at max count in the column
      - "median": weighted median of y in the column (weights = H or counts)
      - "mean": weighted mean of y in the column

    Uncertainty:
      - uses weighted quantiles (q_lo, q_hi) of the column distribution
      - converts to a scatter estimate sigma_y ~ 0.5*(q_hi - q_lo)
      - and stderr ~ sigma_y / sqrt(N_eff) where N_eff = total (unweighted) count in that x-bin,
        or if weights exist, N_eff ≈ (sum w)^2 / (sum w^2).

    Returns:
      x_centers (log10 x), y_ridge (log10 f), y_stderr, counts_x, (optional) details
    """
    H = heatmap_dict["H"]
    H_w2 = heatmap_dict["H_w2"]
    Nraw = heatmap_dict["N"]
    xe = heatmap_dict["x_edges"]
    ye = heatmap_dict["y_edges"]

    x_cent = 0.5 * (xe[:-1] + xe[1:])
    y_cent = 0.5 * (ye[:-1] + ye[1:])

    Nx, Ny = H.shape
    y_ridge = np.full(Nx, np.nan, dtype=float)
    y_stderr = np.full(Nx, np.nan, dtype=float)
    counts_x = Nraw.sum(axis=1)

    # effective sample size for weighted case
    if H_w2 is None:
        Neff = counts_x.astype(float)
    else:
        Sw = H.sum(axis=1)
        Sw2 = H_w2.sum(axis=1)
        Neff = np.zeros_like(Sw, dtype=float)
        m = (Sw2 > 0.0) & np.isfinite(Sw) & np.isfinite(Sw2)
        Neff[m] = (Sw[m] * Sw[m]) / Sw2[m]

    for i in range(Nx):
        if counts_x[i] < min_count_per_xbin:
            continue

        col_w = H[i, :]
        if not np.isfinite(col_w).any() or np.all(col_w <= 0.0):
            continue

        if ridge == "mode":
            j = int(np.argmax(col_w))
            y0 = y_cent[j]
        else:
            # normalize weights for stats
            ww = np.where(np.isfinite(col_w), col_w, 0.0)
            if ww.sum() <= 0.0:
                continue
            if ridge == "mean":
                y0 = np.sum(ww * y_cent) / np.sum(ww)
            elif ridge == "median":
                y0 = _weighted_quantile(y_cent, 0.5, sample_weight=ww)
            else:
                raise ValueError("ridge must be one of: mode, median, mean")

        # uncertainty from weighted quantiles in this column
        ww = np.where(np.isfinite(col_w), col_w, 0.0)
        q16, q84 = _weighted_quantile(y_cent, [q_lo, q_hi], sample_weight=ww)
        if np.isfinite(q16) and np.isfinite(q84) and (q84 > q16) and (Neff[i] > 1.0):
            sigma = 0.5 * (q84 - q16)
            y_stderr[i] = sigma / np.sqrt(Neff[i])
        y_ridge[i] = y0

    if not return_full:
        return x_cent, y_ridge, y_stderr, counts_x
    ...
    return x_cent, y_ridge, y_stderr, counts_x, details

def ridge_bins_for_mpfit_from_points(
    h,
    log10_f,
    log10_f_err_initial=None,
    percentile_split=(0.0, 0.01, 0.99, 1.0),
    n_bins_arr=(10, 100, 10),
    x_is_geomspace=True,
    n_y_bins=160,
    ...
    ridge="mode",
    weights=None,
    clip_percentiles_y=(0.2, 99.8),
    x_edges_h_ref=None,
):
    """
    Convenience: build heatmap in (log10 h, log10 f), then extract a ridge curve for 1D mpfit.

    Inputs:
      h         : positive h (NOT log10)
      log10_f   : y already log10
      weights   : optional (e.g. particle mass). If None, counts-only.

    Returns:
      h_centers (in original h units, NOT log10),
      f_ridge_log10,
      f_ridge_stderr,
      counts_per_xbin,
      heatmap_dict
    """
    # Build ridge bins using ONLY x-binning (h-bins), not 2D heatmap y-bins.
    # This makes forward-model binning stable across mpfit iterations.
    # ------------------------------------------------------------------
    h = np.asarray(h, dtype=float)
    y = np.asarray(log10_f, dtype=float)

    # choose / reuse x-edges in h-space
    if x_edges_h_ref is None:
        x_edges_h, cut_each, n_bins_each = make_x_edges_percentile_piecewise(
            h,
            percentile_split=percentile_split,
            n_bins_arr=n_bins_arr,
            is_geomspace=x_is_geomspace,
            x_must_be_positive_for_geom=True,
        )
    else:
        x_edges_h = np.asarray(x_edges_h_ref, dtype=float)

    nbins = max(0, len(x_edges_h) - 1)
    # preallocate (fixed length => safe for mpfit)
    hbd = np.full(nbins, np.nan, dtype=float)
    fbd = np.full(nbins, np.nan, dtype=float)
    febd = np.full(nbins, np.nan, dtype=float)
    counts_x = np.zeros(nbins, dtype=int)

    # fast bin assignment
    # (ignore h<=0 and non-finite; ignore non-finite y for ridge stats)
    m_h = np.isfinite(h) & (h > 0.0)
    if not np.any(m_h) or nbins == 0:
        hm = {"x_edges_h": x_edges_h}
        return hbd, fbd, febd, counts_x, hm
    ...
    bin_idx = np.searchsorted(x_edges_h, h_use, side="right") - 1
    in_range = (bin_idx >= 0) & (bin_idx < nbins)
    if not np.any(in_range):
        hm = {"x_edges_h": x_edges_h}
        return hbd, fbd, febd, counts_x, hm

    h_use = h_use[in_range]
    y_use = y_use[in_range]
    bin_idx = bin_idx[in_range]
    if w_use is not None:
        w_use = w_use[in_range]

    # for each x-bin, compute ridge(y) and stderr
    for i in range(nbins):
        sel_i = (bin_idx == i)
        if not np.any(sel_i):
            continue
        # keep only finite y for ridge statistics
        yi = y_use[sel_i]
        hi = h_use[sel_i]
        if w_use is not None:
            wi = w_use[sel_i]
        ...
        # count for x-bin (raw count in this x bin)
        counts_x[i] = int(np.sum(sel_i))

        # x representative: mean h in bin
        if np.any(np.isfinite(hi)):
            hbd[i] = np.nanmean(hi)

        # ridge and uncertainty require finite y
        mf = np.isfinite(yi)
        if np.sum(mf) < max(1, int(min_count_per_xbin)):
            continue
        yi = yi[mf]
        ...
        if ridge == "median":
            ...
            # approximate mode by histogram peak (stable, no y-edges dependence)
            nb = max(10, int(np.sqrt(len(yi))))
            hh, ee = np.histogram(yi, bins=nb)
            jj = int(np.argmax(hh))
            y0 = 0.5 * (ee[jj] + ee[jj + 1])
        else:
            # fallback
            y0 = np.median(yi)
        fbd[i] = y0

        # stderr: use 16-84 scatter / sqrt(N)
        q16, q84 = np.percentile(yi, [16.0, 84.0])
        if np.isfinite(q16) and np.isfinite(q84) and (q84 > q16):
            sigma = 0.5 * (q84 - q16)
            febd[i] = sigma / np.sqrt(float(len(yi)))

    # return also the x-edges so caller can reuse it for forward-model bins
    hm = {"x_edges_h": x_edges_h}
    return hbd, fbd, febd, counts_x, hm

# ---------------------------------------------------------
# 4) Fit uncertainty: bootstrap (works with mpfit or any fit)
# ---------------------------------------------------------
def bootstrap_fit_pairs(
    x,
    y,
    yerr,
    fit_callback,
    ...
    """
    Bootstrap uncertainty for 1D fit.
    fit_callback(xb, yb, yerrb) -> params (1D array).
    Returns:
      params_samples (n_boot, n_param),
      summary dict with mean/std and 16-84 percentiles.
    """
    x = np.asarray(x, dtype=float)
    ...
    if x.size < 5:
        raise ValueError("Too few valid points for bootstrap.")

    if rng is None:
        rng = np.random.default_rng()

    # fit once to get n_param
    p0 = fit_callback(x, y, yerr)
    p0 = np.asarray(p0, dtype=float)
    n_param = p0.size
    samples = np.full((n_boot, n_param), np.nan, dtype=float)
    ...
    for b in range(n_boot):
        ...
        xb = x[idx]
        yb = y[idx]
        yeb = yerr[idx] if yerr is not None else None
        try:
            pb = fit_callback(xb, yb, yeb)
            samples[b, :] = np.asarray(pb, dtype=float)
        except Exception:
            # leave NaN for this bootstrap draw
            continue

    good = np.all(np.isfinite(samples), axis=1)
    samples_g = samples[good]
    if samples_g.shape[0] < max(10, n_boot // 5):
        raise RuntimeError("Too many bootstrap failures; fit_callback may be unstable.")

    summary = {
        "mean": np.nanmean(samples_g, axis=0),
        "std":  np.nanstd(samples_g, axis=0, ddof=1),
        "p16":  np.nanpercentile(samples_g, 16, axis=0),
        "p84":  np.nanpercentile(samples_g, 84, axis=0),
        "n_good": samples_g.shape[0],
    }
    return samples_g, summary
...
# -------------------------------------------------------------------------
# 5) Optional: residuals for fitting the FULL heatmap with mpfit (Option 2)
# -------------------------------------------------------------------------
def heatmap_residuals_gaussian_ridge(
    heatmap_dict,
    model_func_ylog10,
    params,
    sigma_y=0.15,
    ...
    """
    Build a residual vector from a 2D heatmap using an approximate Gaussian ridge model:
      y | x ~ Normal(mu(x;params), sigma_y)

    - model_func_ylog10(x_h, *params) should return y_log10 predictions at x_h (h, not log10 h)
    - sigma_y is scatter in y (log10 units). 

    Expected counts in each x-bin:
      lambda_ij = N_i * p_ij
      p_ij proportional to Normal(y_center_j | mu_i, sigma_y) * dy, normalized over y.

    Uncertainty:
      - counts-only: sigma_ij ~ sqrt(lambda_ij)
      - weighted heatmap: if H_w2 exists, use sigma_ij ~ sqrt(H_w2) (data-side)
        and residual = (H - lambda)/sigma_ij.

    Returns:
      resid (1D vector), lambda_ij (2D)
    """
    H = heatmap_dict["H"]
    H_w2 = heatmap_dict["H_w2"]
    Nraw = heatmap_dict["N"]
    ...
    x_cent_log = 0.5 * (xe[:-1] + xe[1:])
    y_cent = 0.5 * (ye[:-1] + ye[1:])
    dy = (ye[1:] - ye[:-1])

    # model is in h (not log10 h)
    h_cent = 10.0 ** x_cent_log
    mu = model_func_ylog10(h_cent, *params)  # (Nx,)

    Nx, Ny = H.shape
    lam = np.zeros_like(H, dtype=float)

    # build p(y|x) per x bin
    for i in range(Nx):
        if Nraw[i, :].sum() < min_count_per_xbin:
            continue
        if not np.isfinite(mu[i]):
            continue
        z = (y_cent - mu[i]) / max(sigma_y, 1e-6)
        pdf = np.exp(-0.5 * z * z) / (np.sqrt(2.0 * np.pi) * max(sigma_y, 1e-6))
        p = pdf * dy
        s = p.sum()
        if s <= 0.0 or (not np.isfinite(s)):
            continue
        p /= s
        Ni = H[i, :].sum()  # if weighted, this is sum of weights
        lam[i, :] = Ni * p

    # residuals
    if H_w2 is not None:
        # weighted-data uncertainty: sqrt(sum w^2)
        sig = np.sqrt(np.maximum(H_w2, eps))
        r = (H - lam) / sig
        r = r[np.isfinite(r)]
        return r.ravel(), lam
    ...
    # counts-only
    if use_deviance:
        # Poisson deviance residuals (good for low counts)
        # sign(H-lam) * sqrt(2*(H*log(H/lam) - (H-lam)))
        lam2 = np.maximum(lam, eps)
        H2 = np.maximum(H, 0.0)
        term = np.where(H2 > 0.0, H2 * np.log(H2 / lam2) - (H2 - lam2), -(H2 - lam2))
        r = np.sign(H2 - lam2) * np.sqrt(2.0 * np.maximum(term, 0.0))
    else:
        # Pearson residuals
        lam2 = np.maximum(lam, eps)
        r = (H - lam2) / np.sqrt(lam2)
