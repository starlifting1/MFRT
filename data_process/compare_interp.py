...
def plot_compare_interp():
    filename1 = "/home/darkgaia/0prog/Stackel/tact-master/DataInteerface/kdtree_knn/20201024_forces_test1.txt"
    A1 = np.loadtxt(filename1, dtype=float)

    f0 = A1[:,0:3] #forces gadget accelerations
    f1 = A1[:,3:6] #forces interp
    f2 = A1[:,6:9] #forces simply add
    ...
    plt.figure()
    plt.scatter(x, f0[:,0], color="black", s=5., label="forces gadget accelerations")
    plt.scatter(x, f1[:,0], color="blue" , s=5., label="forces interp")
    plt.scatter(x, f2[:,0], color="red"  , s=5., label="forces simply add")
    ...
    plt.title("compare_interp forces:\nposition = {data_nodes}", fontsize=20)
