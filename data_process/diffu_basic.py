...
def calculate_fixed_const_coefs(N_particles, M_total, R0, v0):
    ...
    Diffu_ref = diffusion_reference_value_Diffu_ref(N_particles, M_total, R0, v0)
...
def eta_ralaxaiton_time_ratio_frac(N_particles, M_total, R0, v0, Dim_frac):
    """
    Relaxation-time ratio eta_frac corresponding to eq_eta_frac in the tex.
    Note the mass in calculating Diffu should be m = M_total/N_particles.
    """
    Rm = R0 / lambda2

    # print("Calculate fractal eta with N pos.")
    prefactor = (
        (Rm**2 * v0**4) /
        (2.0 * lambda1 * lambda3 * G**2 * M_total**2)
        * np.sqrt(np.pi / 6.0) / GXX
    )
    ...
    if np.isclose(D, 2.0):
        scaling = N / (2.0 * np.log((lambda4 / lambda2) * N))
    elif D > 2.0:
        scaling = (D - 2.0) * N / D
    else:
        scaling = (
            4.0 * np.sin(np.pi * D / 2.0) / (np.pi * D**2)
            * (lambda2 / lambda4)**(2.0 - D)
            * N**(D - 1.0)
        )
