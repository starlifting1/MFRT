...
name_MG = sys.argv[4] #to be the "default" or galaxy_ganeral or galaxy_general_... or multi strings
gm_name = None
...
def _fit_once_unweighted_mpfit(fit_function, fit_settings, xdata, ydata):
    """
    One unweighted MP_fit call.
    fit_settings must be [p0, bounds].
    """
    if fit_settings is None:
        raise ValueError("fit_settings is None; expected [p0, bounds].")
    p0 = np.array(fit_settings[0], dtype=float)
    bounds = [
        np.array(fit_settings[1][0], dtype=float),
        np.array(fit_settings[1][1], dtype=float),
    ]
    quiet = 1
    MF = fgw.MP_fit(
        fit_function, p0,
        xdata=xdata, ydata=ydata, errdata=None, bounds=bounds,
        maxiter=1000, quiet=quiet
    )
    try:
        p = MF.run(is_fit_log=False)
    except TypeError:
        p = MF.run()
    return np.asarray(p, dtype=float)

def _bootstrap_action_fit_on_particles(
    tgts_sub,
    DF_sub,
    fit_function,
    fit_settings,
    ...
    """
    Bootstrap on particles, rebuild bins, then refit.
    Returns a JSON-safe dict or None if bootstrap cannot be constructed.
    """
    tgts_sub = np.asarray(tgts_sub, dtype=float)
    DF_sub = np.asarray(DF_sub, dtype=float).reshape(-1)
    xgrid = np.asarray(xgrid, dtype=float).reshape(-1)

    if tgts_sub.ndim != 2 or tgts_sub.shape[0] != DF_sub.shape[0]:
        raise ValueError("bootstrap: inconsistent shapes of tgts_sub and DF_sub.")
    if tgts_sub.shape[0] < 16:
        return None
    ...
    # fixed x-grid for uncertainty band
    xgrid = xgrid[np.isfinite(xgrid) & (xgrid > 0.0)]
    if xgrid.size == 0:
        return None
    ...
    # parameter count from one fit
    params0 = None
    try:
        h0 = auxiliary_function_plot2d_x(tgts_sub, *_aux_args)
        y0 = np.log10(DF_sub)
        m0 = np.isfinite(h0) & (np.asarray(h0) > 0.0) & np.isfinite(y0)
        hbd0, fbd0, _, _, _ = ads.split_various_bin_1d_percentile(
            np.asarray(h0)[m0], np.asarray(y0)[m0], None,
            percentile_split_used, N_bins_arr_used, is_geomspace_used
        )
        mb0 = np.isfinite(hbd0) & (hbd0 > 0.0) & np.isfinite(fbd0)
        if np.sum(mb0) >= 5:
            params0 = _fit_once_unweighted_mpfit(
                fit_function, fit_settings, hbd0[mb0], fbd0[mb0]
            )
    except Exception:
        params0 = None
    if params0 is None:
    ...
    for _ in range(int(n_boot)):
        ...
        try:
            h_b = auxiliary_function_plot2d_x(tgts_b, *_aux_args)
            y_b = np.log10(DF_b)
            m_b = np.isfinite(h_b) & (np.asarray(h_b) > 0.0) & np.isfinite(y_b)
            if np.sum(m_b) < 16:
                continue

            hbd_b, fbd_b, _, _, _ = ads.split_various_bin_1d_percentile(
                np.asarray(h_b)[m_b], np.asarray(y_b)[m_b], None,
                percentile_split_used, N_bins_arr_used, is_geomspace_used
            )
            mb = np.isfinite(hbd_b) & (hbd_b > 0.0) & np.isfinite(fbd_b)
            if np.sum(mb) < 5:
                continue

            p_b = _fit_once_unweighted_mpfit(
                fit_function, fit_settings, hbd_b[mb], fbd_b[mb]
            )
            if len(p_b) != n_param or (not np.all(np.isfinite(p_b))):
                continue

            y_bfit = fit_function(xgrid, *p_b)
            y_bfit = np.asarray(y_bfit, dtype=float)
            if y_bfit.shape != xgrid.shape:
                continue

            params_boot.append(p_b)
            yfit_boot.append(y_bfit)
        except Exception:
            continue

    if len(params_boot) < max(8, int(n_boot // 4)):
        return None

    params_boot = np.asarray(params_boot, dtype=float)
    yfit_boot = np.asarray(yfit_boot, dtype=float)

    out = {
        "n_boot_good": int(params_boot.shape[0]),
        "params_mean": np.nanmean(params_boot, axis=0).tolist(),
        "params_std": np.nanstd(params_boot, axis=0, ddof=1).tolist(),
        # "params_p16": np.nanpercentile(params_boot, 16.0, axis=0).tolist(),
        # "params_p84": np.nanpercentile(params_boot, 84.0, axis=0).tolist(),
        "params_p16": np.nanpercentile(params_boot, 2.5, axis=0).tolist(),
        "params_p84": np.nanpercentile(params_boot, 97.5, axis=0).tolist(),
        "xgrid": xgrid.tolist(),
        "yfit_p16": np.nanpercentile(yfit_boot, 2.5, axis=0).tolist(),
        "yfit_p84": np.nanpercentile(yfit_boot, 97.5, axis=0).tolist(),
    }
    return out
...
def main_step5_mpfit_fit_DF_AA_one(
    ...
    # selected-sample mass for DF normalization
    if mass is None:
        raise SystemExit("mass is None: cannot set M0 from selected-particle mass.")
    M0_selected_fit = float(np.sum(mass))

    #: processed data, tables for params, with axisRatio, ds, rs, etc.
    DF_unit = DF / M0_selected_fit
    vinp, vout_data = tgts, np.log10(DF_unit) #xdata and ydata after log10
    ...
    paramsresult2.update({"M0_selected_fit": M0_selected_fit})
    ...
    # bd = bd2
    # h, vout = ads.screen_remove_far_back(h, vout_data, h_far=h_far_cut, is_mask_log10f=False) #optional if back bad points and write
    vout = vout_data
    ...
    ) #note that now it is unweighted fitting
    ...
    return DL, combmodel, fitmodelfunc, paramsresult2, hbd, fbd, tgts, DF, M0_selected_fit
...
def plot_1d_fit_massdensity(
    # rq_data, DF_data_log10, rq_data_bin, DF_data_bin_log10, 
    # DF_fitvalue_log10=None, pathfig=None,
    rq_data, DF_data_log10,
    rq_data_bin, DF_data_bin_log10,
    DF_fitvalue_log10=None, pathfig=None,
    heatmap_nx=60, heatmap_ny=40,
):
    """
    Plot mass-density fit in (r_q, log10 rho) space.

    Background: 2D histogram heatmap of particle points in (log10 r_q, log10 rho).
      - We bin uniformly in log10(r_q), but display x-axis as r_q with log scaling,
        so the dilute tail is not visually crushed.
    Foreground: ridge bins + optional fit bins.
    """
    ...
    rq_data = np.asarray(rq_data, dtype=float)
    DF_data_log10 = np.asarray(DF_data_log10, dtype=float)

    # --- heatmap (replaces raw-point scatter) ---
    m = np.isfinite(rq_data) & (rq_data > 0.0) & np.isfinite(DF_data_log10)
    if np.any(m):
        xlog = np.log10(rq_data[m])      # binning coordinate
        ylog = DF_data_log10[m]          # already log10(rho)

        # robust bounds (avoid wasting bins on extreme outliers)
        if xlog.size >= 50:
            xlo, xhi = np.percentile(xlog, [0.2, 99.8])
        else:
            xlo, xhi = np.nanmin(xlog), np.nanmax(xlog)
        if ylog.size >= 50:
            ylo, yhi = np.percentile(ylog, [0.2, 99.8])
        else:
            ylo, yhi = np.nanmin(ylog), np.nanmax(ylog)
        ...
        # x bins uniform in log10(rq), but we display x in rq with log axis
        x_edges_log = np.linspace(float(xlo), float(xhi), int(heatmap_nx) + 1)
        x_edges = 10.0 ** x_edges_log
        y_edges = np.linspace(float(ylo), float(yhi), int(heatmap_ny) + 1)

        H, _, _ = np.histogram2d(xlog, ylog, bins=[x_edges_log, y_edges])

        # clip vmax so the dense core doesn't dominate the color stretch
        Hnz = H[H > 0]
        if Hnz.size > 0:
            vmax = float(np.percentile(Hnz, 99.5))
            vmax = max(vmax, 1.0)
        else:
            vmax = 1.0

        # Hm = np.ma.masked_less(H, 1.0)  # hide empty bins
        Hm = np.ma.masked_less(H, 5.0)  # hide empty bins
        pcm = plt.pcolormesh(
        ...
        # keep the old legend entry name (proxy handle)
        plt.scatter([], [], label="data bins heatmap", s=0.0)
    else:
        plt.text(
            0.05, 0.95, "No finite points for heatmap.",
            transform=plt.gca().transAxes
        )
        plt.scatter([], [], label="data bins heatmap", s=0.0)

    # ridge bins + optional fit bins
    rq_data_bin = np.asarray(rq_data_bin, dtype=float)
    DF_data_bin_log10 = np.asarray(DF_data_bin_log10, dtype=float)
    mb = np.isfinite(rq_data_bin) & (rq_data_bin > 0.0) & np.isfinite(DF_data_bin_log10)
    if np.any(mb):
        plt.scatter(rq_data_bin[mb], DF_data_bin_log10[mb], label="data bins ridge", s=2.0, zorder=3)

    if DF_fitvalue_log10 is not None:
        DF_fitvalue_log10 = np.asarray(DF_fitvalue_log10, dtype=float)
        mf = mb & np.isfinite(DF_fitvalue_log10)
        if np.any(mf):
            plt.scatter(rq_data_bin[mf], DF_fitvalue_log10[mf], label="fit", s=0.5, zorder=3)
    ...
    # plt.savefig(pathfig)
    fig_tmp = plt.gcf()
    savename_fig = pathfig + ".pdf"
    fig_tmp.savefig(
        savename_fig, format="pdf", bbox_inches="tight", 
...
def plot_simple_actions_fitting(
    ...
    M0_selected_fit=None, 
    JO1=None, DF1_log10=None, is_fit_1d=True, 
    x_lim_down=None, x_lim_up=None, 
    fit_uncertainty=None, 
    heatmap_nx=60, heatmap_ny=40,
    is_show_heatmap_band=True,
    heatmap_band_percentiles=(16.0, 84.0),
    heatmap_band_mincount=10,
):
    _aux_args = auxiliary_function_plot2d_args
    ...
    from matplotlib.colors import LogNorm
    # Heatmap of data points in (log10 h, log10 f) space
    xd = np.asarray(xd, dtype=float)
    yd = np.asarray(yd, dtype=float)
    m = np.isfinite(xd) & (xd > 0.0) & np.isfinite(yd)
    if np.any(m):
        hvals = xd[m]
        ylog  = yd[m]
        xlog  = np.log10(hvals)

        # robust bounds but keep the dilute tail better
        #\ use more aggressive percentiles so the tail is not cut away
        if xlog.size >= 50:
            xlo, xhi = np.percentile(xlog, [0.05, 99.95])
        else:
            xlo, xhi = np.nanmin(xlog), np.nanmax(xlog)
        if ylog.size >= 50:
            ylo, yhi = np.percentile(ylog, [0.05, 99.95])
        else:
            ylo, yhi = np.nanmin(ylog), np.nanmax(ylog)
        ...
        if (JOb is not None) and (DF_log10 is not None):
            xdb = np.asarray(JOb, dtype=float)
            ydb = np.asarray(DF_log10, dtype=float)
            mb = np.isfinite(xdb) & (xdb > 0.0) & np.isfinite(ydb)
            if np.any(mb):
                xlo = min(xlo, np.nanmin(np.log10(xdb[mb])))
                xhi = max(xhi, np.nanmax(np.log10(xdb[mb])))
                ylo = min(ylo, np.nanmin(ydb[mb]))
                yhi = max(yhi, np.nanmax(ydb[mb]))

        # clamp bin numbers to avoid "too dense pixels" vs "too blocky tail"
        nx = int(np.clip(int(heatmap_nx), 120, 220))
        ny = int(np.clip(int(heatmap_ny), 90, 180))

        # x edges in h (log-spaced), y edges in log10(f) (linear)
        x_edges = np.logspace(xlo, xhi, nx + 1)
        y_edges = np.linspace(ylo, yhi, ny + 1)
        H, _, _ = np.histogram2d(hvals, ylog, bins=[x_edges, y_edges])

        # plot density (log scale in counts), clip vmax so core doesn't dominate
        Hnz = H[H > 0]
        if Hnz.size > 0:
            vmax = float(np.percentile(Hnz, 99.5))
            vmax = max(vmax, 1.0)
        ...
        # band from particle-count heatmap:
        # for each x-bin, show the central percentile range of log10(f)
        if is_show_heatmap_band:
            qlo, qhi = heatmap_band_percentiles
            y_cent = 0.5 * (y_edges[:-1] + y_edges[1:])
            x_cent = np.sqrt(x_edges[:-1] * x_edges[1:])   # geometric center for log-x plot
            y_lo_band = np.full(len(x_cent), np.nan, dtype=float)
            y_hi_band = np.full(len(x_cent), np.nan, dtype=float)
            n_col = np.sum(H, axis=1)

            for i_col in range(H.shape[0]):
                wcol = H[i_col, :]
                if np.sum(wcol) < heatmap_band_mincount:
                    continue
                qv = _weighted_quantile_1d(
                    y_cent, wcol, [qlo / 100.0, qhi / 100.0]
                )
        ...
        # fallback: nothing finite to show, keep empty plot but do not crash
        plt.text(0.05, 0.95, "No finite points for heatmap.", transform=plt.gca().transAxes)

    if is_fit_1d:
        # xdb = JOb
        # ydb = DF_log10
        # yfb = fit_function(JOb, *fit_params_list)
        # plt.scatter(xdb, ydb, label="data bins ridge", s=1.)
        # plt.scatter(xdb, yfb, label="fit bins", s=1.)
        xdb = np.asarray(JOb, dtype=float)
        ydb = np.asarray(DF_log10, dtype=float)
        mb = np.isfinite(xdb) & (xdb > 0.0) & np.isfinite(ydb)
        if np.any(mb):
            ydb_use = ydb[mb] + np.log10(M0_selected_fit)
            plt.scatter(xdb[mb], ydb_use, color="orange", label="data bins ridge", s=pointsize, zorder=3)

            yfb = fit_function(xdb[mb], *fit_params_list) + np.log10(M0_selected_fit)
            yfb = np.asarray(yfb, dtype=float)
            # plt.scatter(xdb[mb], yfb, color="red", label="fit", s=pointsize, zorder=3)
            plt.plot(xdb[mb], yfb, color="red", label="fit", lw=pointsize/2, zorder=3)

        if fit_uncertainty is not None:
            try:
                xg = np.asarray(fit_uncertainty["xgrid"], dtype=float)
                y16 = np.asarray(fit_uncertainty["yfit_p16"], dtype=float)
                y84 = np.asarray(fit_uncertainty["yfit_p84"], dtype=float)
                mg = np.isfinite(xg) & (xg > 0.0) & np.isfinite(y16) & np.isfinite(y84)
                if np.any(mg):
                    plt.fill_between(
                        xg[mg], y16[mg], y84[mg],
                        color="red", alpha=0.2, linewidth=0.0,
                        # label="bootstrap uncertainty band 16-84%",
                        label="bootstrap uncertainty band 2.5-97.5%",
                    )
            except Exception:
                pass
    else:
        ...
        yf = fit_function(JO1, *fit_params_list) + np.log10(M0_selected_fit)  # may contain NaN
        yf = np.asarray(yf, dtype=float)
        mm = np.isfinite(xd) & (xd > 0.0) & np.isfinite(yf)
        if np.any(mm):
            # plt.scatter(np.log10(xd[mm]), yf[mm], label="fit", s=2.)
            ydb_use = ydb[mb] + np.log10(M0_selected_fit)
            plt.scatter(xdb[mb], ydb_use, color="orange", label="data bins ridge", s=pointsize, zorder=3)
    ...
    xdown_input = xmin_current if x_lim_down is None else x_lim_down
    xup_input   = xmax_current if x_lim_up   is None else x_lim_up
    ax.set_xlim(xdown_input, xup_input)

    dy = np.log10(M0_selected_fit)
    ymin_now, ymax_now = ax.get_ylim()
    ax.set_ylim(ymin_now + dy, ymax_now)

    plt.legend()
    plt.xscale("log")
    plt.xlabel(r"value of combination of actions, $h(\mathbf{J})$ ($\mathrm{kpc\, km/s)}$")
    # plt.ylabel(r"$\log_{10}{f(\mathbf{J}})$ ($\mathrm{M_\mathrm{sel}/(kpc\, km/s)^3)}, M_\mathrm{sel}=%(.2f)"%(M0_selected_fit))
    plt.ylabel(
    ...
    #     plt.show()
    plt.close("all")
    print("Plot 1d AA DF and save file to %s, done." % (savename_fig))
    return 0
...
def get_fit_choice(yaml_path, particle_type):
    ...
    Args:
      ...
      particle_type: int (e.g. Gadget type)
...
if __name__ == '__main__':
    ...
    if tag_task==1: #xv triaxialization
        ...
        for gadget_type in type_list:
            ...
            #(2) fit actions DF
            DL, combmodel, fitmodelfunc, paramsresult, hbd, fbd, tgts_sub, DF_sub, M0_selected_fit = \
            ...
            fitresult = fitmodelfunc[0][4] #note that the DF is for unit mass, and then the DF is with M0_selected_fit
            ...
            # # bootstrap uncertainty: particles -> rebuild bins -> refit
            # fit_uncertainty = None
            # try:
            #     print("begin bootstrap")
            #     _settings = fitmodelfunc[0][5] if (fitmodelfunc and len(fitmodelfunc[0]) > 5) else None
            #     fit_uncertainty = _bootstrap_action_fit_on_particles(
            #         tgts_sub=tgts_sub,
            #         DF_sub=DF_sub,
            #         fit_function=fitmodelfunc[0][0],
            ...
            #         xgrid=np.asarray(hbd, dtype=float),
            #         percentile_split_used=[0., 0.01, 0.99, 1.],
            #         N_bins_arr_used=[10, 100, 10],
            #         is_geomspace_used=True,
            #         n_boot=40,
            #         random_seed=12345 + int(snapshot_ID) + int(gadget_type),
            #     )
            #     print("end bootstrap")
            #     if fit_uncertainty is not None:
            #         meta["fit_uncertainty_bootstrap"] = fit_uncertainty
            # except Exception as e:
            #     print("Warning: bootstrap fit uncertainty failed:", e)
            ...
            #note: tgts and DF here are already filtered inside the call, and one passes the filtered ones
            pathfig = path_file_data_types + ".png"
            fit_uncertainty = None
            x_lim_down = None
            ...
            plot_simple_actions_fitting(
                ...
                tgts_sub, np.log10(DF_sub), 
                is_fit_1d=is_fit_1d, fit_uncertainty=fit_uncertainty,
                x_lim_down=x_lim_down, x_lim_up=x_lim_up, 
            )
