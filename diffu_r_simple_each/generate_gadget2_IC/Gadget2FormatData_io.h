...
public:
    ...
    * particle data.
    */
    int allocate_memory_big_int(int N_generate);
    ...
    */
    ...
    void write_samples_to_binary(const std::vector<PositionSpace>& pos, const std::vector<VelocitySpace>& vel, const std::string& filename) {
    ...
    // Function to write samples into IC
    void write_samples_to_PD(const std::vector<PositionSpace>& pos, const std::vector<VelocitySpace>& vel) {
        size_t N_genrate = pos.size();
        for (size_t i = 0; i < N_genrate; ++i) {
            P[i+1].Pos[0] = pos[i].x;
            P[i+1].Pos[1] = pos[i].y;
    ...
    // Function to read IC into pos and vel
    void write_PD_to_xvCartesian(std::vector<PositionSpace>& pos, std::vector<VelocitySpace>& vel) {
        // std::cout<<"NumPart by loading: "<<NumPart<<"\n";
        std::cout<<"NumPartTot by loading: "<<NumPartTot<<"\n";
        pos.resize(NumPartTot);
        vel.resize(NumPartTot);
        for (size_t i = 0; i < NumPartTot; ++i) {
            pos[i].x = P[i+1].Pos[0];
            pos[i].y = P[i+1].Pos[1];
            pos[i].z = P[i+1].Pos[2];
            vel[i].x = P[i+1].Vel[0];
